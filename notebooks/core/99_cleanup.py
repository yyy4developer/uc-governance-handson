...
# MAGIC > 管理タグの削除は**アカウントレベルの権限**が必要（講師向けの任意セル）
# MAGIC
...
# MAGIC %md
# MAGIC ## 4.（任意・講師向け）管理タグの削除
# MAGIC
# MAGIC 管理タグは**アカウント全体で共有**されるため、上の処理では削除していません。
# MAGIC **全参加者のクリーンアップが終わったあと**、講師が最後に実行してください。
# MAGIC
...
# ★ 管理タグも削除する場合は True に変更（全参加者の完了後、講師のみ）
...
# MAGIC - **ワークスペース自体**: 検証用に作った sandbox なら、不要になった時点で削除を依頼
